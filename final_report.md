# AYUSH Platform — Final Master Technical Overview
...
- **Database**: MongoDB via `motor` (AsyncIOMotorClient).
  - Existing collections: `users`, `food_logs`, `session_logs`, `analysis_captures`, `plant_posts`, `contact_requests`, `recipes`, `yt_cache`, `food_wisdom`, `ojas_history`, `daily_logs`, `checkins`.
...
- **Backend App Structure**:
  - `main.py`: Entry point, lifecycle manager, routers inclusions, and APScheduler startup.
  - `modules/`: Feature-sliced directories (`auth`, `biometrics`, `checkins`, `community`, `food`, `onboarding`, `packaged_food`, `plant`, `predict`, `recipe`, `sos`, `yoga`). Each contains `router.py`, `service.py`, `schemas.py`.
  - `database/`: `mongodb.py` connecting Motor client.
  - `utils/`: `scheduler.py` handles the midnight Cron for data aggregation.
- **Flutter App Structure**: Feature-sliced inside `lib/features/` with layered architecture (`presentation/screens`, `data/repositories`, `providers/`).
  - **State management**: Riverpod & Provider (e.g., `AuthProvider`, `FoodScanProvider`, `CheckinNotifier`).
- **External APIs**:
  ...
  - **YOLOv8**: Local `.pt` model run via `ultralytics` for fresh food scanning.
...
Known issues: Robust error handling implemented.
...
STATUS: WORKING
Backend route: POST /api/v1/checkins
Backend file: modules/checkins/router.py
Flutter screen: daily_checkin_card.dart (in home_screen.dart)
What it does: Captures user sliders for daily health and records them in DB.
What it saves to DB: `checkins` -> `sleep_quality`, `stress_level`, `energy_level`.
Gemini used: NO
YOLO used: NO
MediaPipe used: NO
OpenCV used: NO
Known issues: None, fully connected and successfully drives LSTM pipeline.

FEATURE: Daily Log Consolidation Cron
STATUS: WORKING
Backend route: POST /api/v1/checkins/trigger-cron (Manual override)
Backend file: utils/scheduler.py
Flutter screen: None (Background process)
What it does: APScheduler runs at 00:01 UTC every day. Iterates through all users, fetching the day's food_logs, session_logs, analysis_captures, and checkins. Maps everything to a strict 12-feature schema expected by the LSTM model.
What it saves to DB: `daily_logs` -> `{features, sources_used, consolidated}`.
Gemini used: NO
YOLO used: NO
MediaPipe used: NO
OpenCV used: NO
Known issues: None. Fully operational.
...
What it does: Uses an offline PyTorch LSTM (`ojas_lstm_final.pt`) to forecast OJAS 3 days out based on 7 days of normalized features from `daily_logs`.
What it saves to DB: Updates `daily_logs` -> `prediction.predicted_ojas_day3`.
Gemini used: NO
YOLO used: NO
MediaPipe used: NO
OpenCV used: NO
Known issues: None. Now autonomously fed by the scheduler.
...
**ACTION -> User submits Daily Check-in**
1. **Flutter**: `daily_checkin_card.dart` calls `/api/v1/checkins`.
2. **FastAPI**: `modules/checkins/router.py` handles route.
3. **External API**: None.
4. **MongoDB writes**: Upserts into `checkins` based on `userId` and current `date`.
5. **OJAS update**: NO. It is retrieved later by the Cron job to build `daily_logs`.
6. **Response**: `{status, message}`.
...
- **Created in**: `utils/scheduler.py` (APScheduler job).
...
- **Who writes to it**: Offline cron/pipeline, `utils/scheduler.py`, `predict/router.py`.
...
**COLLECTION: `checkins`**
- **Created in**: `checkins/router.py`.
- **Documents**: `{ userId, date, sleep_quality, stress_level, energy_level, updatedAt }`
- **Who writes to it**: Flutter check-in UI.
...
| POST | `/api/v1/checkins`| `checkins/router.py`| `log_daily_checkin` | NO | WORKING |
| POST | `/api/v1/checkins/trigger-cron`| `checkins/router.py`| `trigger_cron` | NO | WORKING |
...
- **Fix**: Apply `Depends(verify_token)` middleware on all protected routes.

**BUG #2:**
- **Severity**: MEDIUM
- **Description**: Onboarding Bias. Base OJAS is exactly 100. Most users will end up with 85+ on Day 1, which causes the LSTM model to struggle predicting large variances since the baseline is overly optimistic.
- **Fix**: Update the `calculate_ojas` formula to normalize the start to ~70 so the model has room to observe positive momentum.
...
- **What it shows**: User Dashboard, OJAS Score, Daily Check-in Card (sliders).
- **API calls made**: POST `/api/v1/checkins` (via DailyCheckinCard)
...
Based on the latest patches, **ZERO critical gaps remain** for a working Demo.
The pipeline from Flutter UI (Food, Yoga, Checkins, Biometrics) -> API -> MongoDB -> APScheduler (Midnight Cron) -> PyTorch LSTM is entirely closed and functional. 

To improve the demo further, the following non-critical additions could be made:
1. Connecting a Flutter UI for the SOS Fall Detection triggers.
2. Building an interactive historical line chart on the Flutter App to display `ojas_history`.
...
FILES ANALYZED: 122
TOTAL ROUTES FOUND: 48
TOTAL FEATURES FOUND: 13
CRITICAL BUGS FOUND: 2
MISSING FOR DEMO: 0 gaps
